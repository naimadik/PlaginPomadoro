...
function activate(context) {
    ...
    setTimeout(() => {
        vscode.window.showInformationMessage('🍅 Pomodoro Anti-Procrastination готов! Нажми Cmd+Shift+8 для быстрого старта.', 'Быстрый старт (25/5)', 'Настроить время', 'Тест мониторинга').then(choice => {
...
async function quickStart() {
    ...
    const workTime = config.get('workTime', 25);
    const breakTime = config.get('breakTime', 5);
    if (isRunning) {
        const status = isBreak ? 'Перерыв' : 'Работа';
        vscode.window.showInformationMessage(`🍅 Pomodoro уже запущен! ${status}: ${formatTime(timeLeft)} осталось`);
    }
    else {
        if (workTime <= 0 || breakTime <= 0) {
            vscode.window.showErrorMessage('Некорректные настройки времени. Проверьте настройки Pomodoro.');
            return;
        }
...
async function startCustomPomodoro() {
    ...
    try {
        ...
        vscode.window.showInformationMessage(`✅ Настройки сохранены: ${workMinutes} мин работы, ${breakMinutes} мин перерыва`);
...
async function forceCloseTabs() {
    const config = vscode.workspace.getConfiguration('pomodoro');
    const blockedSites = config.get('blockedSites', []);
    try {
...
function startSession(workMinutes, breakMinutes) {
    if (isRunning) {
    ...
    isRunning = true;
    isBreak = false;
    vscode.window.showInformationMessage(`🍅 Pomodoro запущен! ${workMinutes} минут фокусировки.`);
    startMonitoring();
    ...
    timer = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            if (!isBreak) {
                ...
                stopSession();
                vscode.window.showInformationMessage('🎉 Перерыв окончен! Готов к следующей сессии?', 'Начать новую сессию').then(choice => {
...
function startBreak(breakMinutes) {
    ...
    isBreak = true;
    vscode.window.showInformationMessage(`🎉 Рабочая сессия завершена! Начинается ${breakMinutes}-минутный перерыв.`);
    if (monitor) {
        clearInterval(monitor);
        monitor = undefined;
    }
...
function isDistractingWindow(windowInfo) {
    ...
    const isDistracting = blockedSites.some(site => title.includes(site.toLowerCase()));
    ...
    if (autoClose) {
        try {
            ...
            if (blockedSites.length === 0) {
                vscode.window.showWarningMessage('⚠️ Список заблокированных сайтов пуст. Добавьте сайты в настройках.', "Открыть настройки").then(choice => {
            ...
            const result = await closeBrowserTabs(blockedSites);
            if (result.closedCount > 0) {
                vscode.window.showWarningMessage(`🚫 Закрыто ${result.closedCount} отвлекающих вкладок! Фокусируйся на работе!`, "Настройки").then(choice => {
                    if (choice === 'Настройки') {
...
async function showDistractionDialog(shortTitle, app) {
    const messages = [
        "Эй! Ты же в Pomodoro! 🍅",
    ...
    const choice = await vscode.window.showWarningMessage(randomMessage, {
        modal: false,
        detail: `Обнаружено отвлечение: ${app} - ${shortTitle}`
    }, "Закрыть вкладки автоматически", "Я вернулся!", "Игнорировать");
...
async function showSystemAlert(title, message) {
...
async function closeBrowserTabs(blockedSites) {
    let closedCount = 0;
    try {
...
async function closeChromeTabs(blockedSites) {
    let closedCount = 0;
    if (blockedSites.length === 0) {
        return 0;
    }
...
async function closeSafariTabs(blockedSites) {
    let closedCount = 0;
    if (blockedSites.length === 0) {
        return 0;
    }
